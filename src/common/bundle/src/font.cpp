...
void Font::delete_face(FT_Face face) {
    SM_ASSERT_FT2(FT_Done_Face(face), "failed to unload font %s", face->family_name);
}
    ...
    , m_data(std::move(data))
{
    ...
    SM_ASSERT_FT2(FT_New_Memory_Face(ft.get_library(), m_data.data(), FT_Long(m_data.size()), 0, &m_face), "failed to load font %s", m_name);

    SM_ASSERT_FT2(FT_Select_Charmap(*m_face, FT_ENCODING_UNICODE), "failed to select unicode charmap for font %s", m_name);
    ...
    SM_ASSERT_FT2(FT_Set_Char_Size(*m_face, 0, 16 * 64, 300, 300), "failed to set font size for font %s", m_name);
...
math::uint2 Font::get_glyph_size(char32_t codepoint) const {
    SM_ASSERT_FT2(FT_Load_Char(*m_face, codepoint, FT_LOAD_DEFAULT), "failed to load glyph for codepoint %u", int32_t(codepoint));
...
GlyphInfo Font::draw_glyph(char32_t codepoint, math::uint2 start, Image& image, const math::float4& color) const {
    ...
    SM_ASSERT_FT2(FT_Load_Char(*m_face, codepoint, FT_LOAD_RENDER), "failed to load glyph for codepoint %u", int32_t(codepoint));
