...
static const char *get_config_val(const ref_ast_t *ast)
{
    if (ast == nullptr)
        return nullptr;

    ref_ast_t *val = ast->expr;
    CTASSERTF(val != nullptr, "null config value");
...
void Sema::forward_module(ref_ast_t *mod)
{
    ...
    m_api = get_config_val(mod->config[eRefConfigApi]);
    m_vector = get_config_val(mod->config[eRefConfigVector]);
    m_array = get_config_val(mod->config[eRefConfigArray]);
    m_string = get_config_val(mod->config[eRefConfigString]);
    m_string_view = get_config_val(mod->config[eRefConfigStringView]);
...
void Method::emit_impl(Sema& sema, cxx_emit_t *out) const {
    Type *ret = m_return ? m_return->get_type() : new VoidType("void");
    auto it = ret->get_cxx_name(sema, get_name());
    String params;
    String args;
    m_params.foreach([&](auto param) {
        ...
        if (!params.empty())
            params += ", ";
        params += param->get_type()->get_cxx_name(sema, param->get_name());
        args += param->get_name();
    });
...
void Method::emit_method(Sema& sema, cxx_emit_t *out, const RecordType& parent) const {
...
void Method::emit_thunk(Sema& sema, cxx_emit_t *out) const {
    Type *ret = m_return ? m_return->get_type() : new VoidType("void");
    const char *cxxname = get_attrib_string(m_ast->attributes, eAttribCxxName);
    const char* inner = cxxname ? cxxname : refl_fmt("impl_%s", get_name());
    auto it = ret->get_cxx_name(sema, inner);
    String params;
    m_params.foreach([&](auto param) {
        if (!params.empty())
            params += ", ";
...
void RecordType::emit_proto(Sema& sema, cxx_emit_t *out) const
{
...
ref_privacy_t RecordType::emit_methods(Sema& sema, cxx_emit_t *out, ref_privacy_t privacy) const
{
    ...
    m_methods.foreach([&](auto method) {
        ...
        method->emit_method(sema, out, *this);
    });
...
ref_privacy_t RecordType::emit_fields(Sema& sema, cxx_emit_t *out, const Vector<Field*>& fields, ref_privacy_t privacy) const
{
...
const char* IntType::get_cxx_name(Sema& sema, const char *name) const
{
...
{
    const char *privacy = ::get_privacy(m_ast->privacy);
    auto it = get_type()->get_cxx_name(sema, get_name());
    cxx_writeln(out, "%s: %s;", privacy, it);
}

void Field::emit_field(Sema& sema, cxx_emit_t *out) const
{
    auto it = get_type()->get_cxx_name(sema, get_name());
...
void Class::emit_impl(Sema& sema, cxx_emit_t *out) const
{
    if (type_is_external(m_ast))
        return;
    ...
    priv = emit_fields(sema, out, m_fields, priv);
    priv = emit_methods(sema, out, priv);
    emit_end_record(out);
}

void Struct::emit_impl(Sema& sema, cxx_emit_t *out) const
{
    if (type_is_external(m_ast))
        return;
...
void Case::emit_impl(Sema& sema, cxx_emit_t *out) const
{
...
void Variant::emit_impl(Sema& sema, cxx_emit_t *out) const
{
    ...
    if (m_parent)
    {
        ...
        {
            const char *underlying = m_parent->get_cxx_name(sema, nullptr);
            ty = underlying;
    ...
    if (is_facade)
    {
        ...
        {
            cxx_writeln(out, "using facade_t = %s;", m_parent->get_cxx_name(sema, nullptr));
    ...
    emit_methods(sema, out, ePrivacyPublic);
...
void Decl::emit_name_info(Sema& sema, cxx_emit_t *out, const char *id, const ref_ast_t *ast) const
{
    ...
    cxx_writeln(out, "static constexpr ObjectName kDisplayName = impl::objname(\"%s\");", id);
...
void RecordType::emit_serialize_body(cxx_emit_t *out, const char *id, const Vector<Field*>& fields) const
{
    ...
    CTASSERTF(m_parent == nullptr, "cannot serialize a record with a parent");

    cxx_writeln(out, "template<typename Archive>");
    cxx_writeln(out, "constexpr static void serialize(const Type& object, Archive& ar) {");
    cxx_enter(out);
    for (size_t i = 0; i < fields.size(); i++)
    {
        auto f = fields.get(i);
        ...
        cxx_writeln(out, "ar.write(object.%s);", f->get_name());
    }
    cxx_leave(out);
    cxx_writeln(out, "}");

    cxx_writeln(out, "template<typename Archive>");
    cxx_writeln(out, "constexpr static void deserialize(Type& object, Archive& ar) {");
    cxx_enter(out);
    for (size_t i = 0; i < fields.size(); i++)
    {
        auto f = fields.get(i);
        ...
        cxx_writeln(out, "ar.read(object.%s);", f->get_name());
    }
    cxx_leave(out);
    cxx_writeln(out, "}");
}
...
void Class::emit_reflection(Sema& sema, cxx_emit_t *out) const
{
    ...
    auto id = get_decl_name(m_ast, sema, get_name());
    auto parent = m_parent ? m_parent->get_cxx_name(sema, nullptr) : "void";
...
void Variant::emit_serialize(cxx_emit_t *out) const {
    if (!Decl::emit_serialize())
        return;
    ...
    cxx_writeln(out, "constexpr static void serialize(const Type& object, Archive& ar) {");
    cxx_enter(out);
    cxx_writeln(out, "ar.write(object.m_value);");
    cxx_leave(out);
    cxx_writeln(out, "}");
    ...
    cxx_writeln(out, "constexpr static void deserialize(Type& object, Archive& ar) {");
    cxx_enter(out);
    cxx_writeln(out, "ar.read(object.m_value);");
    cxx_leave(out);
    cxx_writeln(out, "}");
}

void Variant::emit_to_string(cxx_emit_t *out, const char *id) const {
    bool is_bitflags = has_attrib_tag(m_ast->attributes, eAttribBitflags);

    cxx_writeln(out, "constexpr static string_t to_string(type_t value, [[maybe_unused]] int base = 10) {");
    cxx_enter(out);
    if (is_bitflags)
    {
...
void Variant::emit_reflection(Sema& sema, cxx_emit_t *out) const
{
    ...
    cxx_writeln(out, "template<> consteval bool is_enum<%s>() { return true; };", id);
