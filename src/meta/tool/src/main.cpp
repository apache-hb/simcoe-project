...
class CmdAfterConsumer : public ASTConsumer {
    ...
    std::ostringstream mHeaderForwards;

    PrettyPrint mHeaderOut;
    PrettyPrint mSourceOut;
    PrettyPrint mHeaderForwardsOut;
    ...
    void handleEnum(const Decl &D, const AnnotateAttr &Attr) {
        ...
        // const ReflectInfo& Info = gReflectInfo.at(&D);
        const EnumDecl &ED = cast<EnumDecl>(D);

        // get the namespace the enum is in so we can forward declare it
        std::string ns = getNamespaceAsString(ED);

        mHeaderForwardsOut.println("namespace {} {{", ns);
        mHeaderForwardsOut.indent();
        mHeaderForwardsOut.println("enum class {};", ED.getNameAsString());
        mHeaderForwardsOut.dedent();
        mHeaderForwardsOut.println("}}");

        mHeaderOut.println("template<>");
        mHeaderOut.println("consteval auto reflect<{}>() {{", ED.getQualifiedNameAsString());
        mHeaderOut.indent();
        mHeaderOut.println("EnumInfo info{{ \"{}\", \"{}\" }};", ED.getNameAsString(), ED.getQualifiedNameAsString());
        mHeaderOut.println("return info;");
        mHeaderOut.dedent();
        mHeaderOut.println("}}");
    }

    void Initialize(ASTContext&) override {
        mHeaderOut.init(&mHeaderBody);
        mSourceOut.init(&mSourceBody);
        mHeaderForwardsOut.init(&mHeaderForwards);

        mHeaderForwardsOut.println("/* This file is generated by the meta reflection tool. */");
        mHeaderForwardsOut.println("#pragma once");
        mHeaderForwardsOut.println("#include \"meta/class.hpp\"");
...
public:
    ...
    ~CmdAfterConsumer() {
        ...
        header.open(gHeaderOutPath);
        source.open(gSourceOutPath);
